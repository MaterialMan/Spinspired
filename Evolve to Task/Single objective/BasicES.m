...
config.error_to_check = 'train&val&test';%'all&connectivity';
...
for test = 1:config.num_tests
    ...
    for gen = 2:config.total_gens
        ...
        if (mod(gen,config.gen_print) == 0)
            msg = strcat('Gen %d, time taken: %.4f sec(s)\n  children:',repmat('%.4f ',1,config.num_children),', parent: %.4f, best Error: %.4f \n');
            fprintf(msg,gen,toc/config.gen_print,store_error(test,gen,2:end),store_error(test,gen,1),getError('test',population(best_indv(test,gen))));
            tic;
