...
config.num_nodes = [repmat(10,1,9)];                   % num of nodes in each sub-reservoir, e.g. if config.num_nodes = [10,5,15], there would be 3 sub-reservoirs with 10, 5 and 15 nodes each.
...
for test = 1:config.num_tests
    ...
    % plot best
    plotReservoirDetails(population,best_indv(test,:),1,best_indv(test,1),config);
...
%             end
        ...
        store_error(test,gen,:) = getError(config.error_to_check,population);
...
save(strcat(res_type,'_',config.dataset,'_run',num2str(config.num_tests),'_gens',num2str(config.total_gens),'_',num2str(config.total_units),'Nres.mat'),...
