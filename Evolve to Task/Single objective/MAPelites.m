...
switch(length(config.metrics))
    ...
    case 2
        ...
        config.combs = [ca(:), cb(:)];
    case 3
        [ca, cb, cc] = ndgrid(0:config.MAP_resolution(config.res_iter):config.total_MAP_size,...
            0:config.MAP_resolution(config.res_iter):config.total_MAP_size,...
            0:config.MAP_resolution(config.res_iter):config.total_MAP_size); %no. of inputs/outputs to ngrid is no. of behaviours
        config.combs = [ca(:), cb(:), cc(:)]; % manually add all combinations together
end
