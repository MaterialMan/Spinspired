...
config.prune_generation = 50;
config.tolerance = [0 0 0];
config.prune_iterations = 20;
...
for test = 1:config.num_tests
    ...
    for gen = 2:config.total_gens
        ...
        if config.multi_offspring
            ...
            if config.prune && (mod(gen,config.prune_generation) == 0)
                %rng(1,'twister')
                ...
                population(indx(1:4))= [Pruned_best_individual{1:4}];
                
                %update errors
                store_error(test,gen,indx(1:4)) = getError(config.error_to_check,population(indx(1:4)));%population(loser).val_error;
                [best(test,gen),best_indv(test,gen)] = min(store_error(test,gen,:));
            end
