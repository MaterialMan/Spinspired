...
function [population,store_error] = basicESFcn(reservoir,task,ID)
...
config.num_nodes = reservoir;                   % num of nodes in each sub-reservoir, e.g. if config.num_nodes = [10,5,15], there would be 3 sub-reservoirs with 10, 5 and 15 nodes each.
...
[best(1),best_indv(1)] = min(getError(config.error_to_check,population));
fprintf('\n Starting loop... Best error = %.4f\n',best(1));

% store error that will be used as fitness in the GA
store_error(1,:) = getError(config.error_to_check,population);
...
for gen = 2:config.total_gens
    ...
    cmp_error = reshape(store_error(gen-1,:),1,size(store_error,2));
    ...
    store_error(gen,:) = getError(config.error_to_check,population);%population(loser).val_error;
    [best(gen),best_indv(gen)] = min(store_error(gen,:));
    
    % print info
    if (mod(gen,config.gen_print) == 0)
        fprintf('Gen %d, time taken: %.4f sec(s)\n  children: %.4f %.4f %.4f, parent: %.4f, best Error: %.4f \n',gen,toc/config.gen_print,store_error(gen,2:end),store_error(gen,1),getError('test',population(best_indv(gen))));
        tic;
