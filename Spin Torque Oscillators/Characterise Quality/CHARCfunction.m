...
% Author: M. Dale
% Date: 03/07/19
function CHARCfunction(num_nodes,structure)
...
config.num_nodes = num_nodes;                  % num of nodes in each sub-reservoir, e.g. if config.num_nodes = {10,5,15}, there would be 3 sub-reservoirs with 10, 5 and 15 nodes each. For one reservoir, sate as a non-cell, e.g. config.num_nodes = 25
...
config.RoR_structure = structure;
