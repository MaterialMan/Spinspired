...
for pop_indx = 1:length(time_scale)
    warning('off','all')
   ...
   % population(pop_indx).input_weights{1}(tmp_config.num_nodes/2 - floor(sqrt(tmp_config.num_nodes)/2),1) = 1;
    ...
    %population(pop_indx).time_steps_increment = time_scale(pop_indx);
    population(pop_indx).interpolation_length = time_scale(pop_indx);
