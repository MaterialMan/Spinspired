...
switch config.dataset
    ...
    case 'secondorder_task' %best 3.61e-3
     ...
     case 'narma_5' %input error 4 - good task
        err_type = 'NRMSE';
    ...
    case {'simplenarx','exchanger','maglev','ph','pollution','refmodel','robotarm','valve'} % tasks taken from Deep Learning toolbox
        ...
        data = load (strcat(config.dataset,'_dataset'));%load('laser.txt');
        input_sequence = cell2mat(data.(strcat(config.dataset,'Inputs')));
        output_sequence = cell2mat(data.(strcat(config.dataset,'Targets')));

        ahead = 1;
        input_sequence = input_sequence(:,1:end-ahead)';
        % plus previous target signal
        input_sequence = [input_sequence output_sequence(:,1:end-ahead)'];
        output_sequence = output_sequence(:,ahead+1:end)';

    case {'chickenpox','ice','laser','oil','river','solar'} % tasks taken from Deep Learning toolbox
        ...
        data = load (strcat(config.dataset,'_dataset'));%load('laser.txt');
        data = cell2mat(data.(strcat(config.dataset,'Targets')));
    ...
    case 'synthetic_digit'
        ...
        config.train_input_sequence = XTrain;
        config.train_output_sequence = YTrain';
        config.val_input_sequence = [];
        config.val_output_sequence = [];
        config.test_input_sequence = XTest;
        config.test_output_sequence = YTest';

        config.wash_out = wash_out;
        config.err_type = err_type;

        config.task_num_inputs = size(config.train_input_sequence{1}',2);
    ...
    case 'cifar10'
        ...
        %% Text classification
    case 'text_classify_factory' % not finished
        ...
        XTest = doc2sequence(enc,documentsValidation,'Length',sequenceLength);
        ...
        % squash into structure
        config.train_input_sequence = XTrain';
        config.train_output_sequence = onehotencode(categorical(YTrain),2);
        config.val_input_sequence = [];
        config.val_output_sequence = [];
        config.test_input_sequence = XTest';
        config.test_output_sequence = onehotencode(categorical(YTest),2);

        config.wash_out = wash_out;
        config.err_type = err_type;

        config.task_num_inputs = inputSize;
        config.task_num_outputs = numClasses;
    ...
    case 'japanese_vowels' %(12: IN, 9:OUT - binary ) - input only 83% accuracy!  Train:0.2288  Test:0.1863
        ...
        t1 =  randperm(size(XTrain,1));
        t2 =  randperm(size(XTest,1));

        XTrain = {XTrain{t1}};
        YTrain = double(YTrain(t1));

        XTest = {XTest{t2}};
        YTest = double(YTest(t2));
    ...
    case 'FSDD' % Free spoken digit recognition database - perform classification using wavelet time scattering
        ...
        tempdir = '/media/sf_test-CHARC/branches/multilayer/Support files/other/Datasets/';
        
        if ~isfile(strcat(tempdir,'FSDD_traintest.mat'))
            ...
            % transform and truncate/pad data - helper function helperReadSPData truncates or pads the data to a length of 8192 and normalizes each recording by its maximum value
            trainWB = waitbar(0,'Getting training data...');
            Xtrain = []; i = 1; Nt = 1600;
            ...
            while hasdata(scatds_Train)
                ...
                if mod(i,floor(Nt/10)) == 0
                    waitbar(i/Nt,trainWB);
                end
            end

            testWB = waitbar(0,'Getting test data...');
            i = 1; Nt = 400;
            Xtest = [];
            ...
            % save data
            save(strcat(tempdir,'FSDD_TrainTest.mat'),'TrainFeatures','TestFeatures','ads');
        else
            load(strcat(tempdir,'FSDD_TrainTest.mat'));
        end

        % squash into structure
        config.train_input_sequence = TrainFeatures;
        config.train_output_sequence = onehotencode(categorical(ads.Labels(1:1600)),2);
        config.val_input_sequence = [];
        config.val_output_sequence = [];
        config.test_input_sequence = TestFeatures;
        config.test_output_sequence = onehotencode(categorical(ads.Labels(1601:end)),2);
    ...
    case 'diabetes'
        ...
        % breast cancer
        err_type = 'softmax';
        config.train_fraction=0.75;    config.val_fraction=0;    config.test_fraction=0.25;

        T = readtable('diabetes.csv');

        input_sequence = table2array(T(:,1:end-1));
        output_sequence = onehotencode(categorical(T.Outcome),2);
    ...
    case 'california_housing'
        ...
        err_type = 'RMSE';
        wash_out = 0;
        config.train_fraction=0.4;    config.val_fraction=0;    config.test_fraction=0.6;

        T = readtable('housing.csv');
        input_sequence = T;
        input_sequence(:,{'median_house_value','ocean_proximity'}) = [];

        % one hot encode ocean proximity
        proximity = onehotencode(categorical(T.ocean_proximity),2,"ClassNames",unique(T.ocean_proximity));
        
        input_sequence = table2array(input_sequence);
        %input_sequence = [input_sequence proximity];
        output_sequence = T.median_house_value; 
      
        % remove NaNs
        nan_pos = any(isnan(input_sequence), 2);
        input_sequence(nan_pos,:) = [];
        output_sequence(nan_pos,:) = [];
...
if normalDataSplit
    ...
    switch(config.input_mechanism)
        case'spiking'
            ...
            [input_sequence,~,config.filter,output_sequence]= encodeBSAspike(input_sequence, config.filter,output_sequence);
