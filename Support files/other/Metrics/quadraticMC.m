function quad_MC = quadraticMC(individual,config,seed)

rng(seed,'twister');

n_internal_units = individual.total_units;

n_output_units = n_internal_units*2;
n_input_units = individual.n_input_units;

%% Assign input data and collect target output
data_length = n_output_units*3 + config.wash_out*2; 
sequence_length = (data_length-n_output_units)/2;%200; 
data_sequence = 2*rand(n_input_units,data_length)-1;

% rescale for each reservoir
%[data_sequence] = featureNormailse(data_sequence,config);
            
if config.discrete %strcmp(config.res_type,'elementary_CA') || strcmp(config.res_type,'2d_CA') || strcmp(config.res_type,'RBN')
    data_sequence = floor(heaviside(data_sequence));
end

input_sequence = data_sequence';%(n_output_units+1:data_length+n_output_units)';

%% quadratic memory capacity - y_k(n)=3u^2(n-k)-1 
quad_input_sequence = input_sequence;
for k = 1:n_output_units
    for n = 1:length(input_sequence)
        if n > k
            quad_output_sequence(n,k) = (3.*(quad_input_sequence(n-k,1).^2)) - 1;
        else
            quad_output_sequence(n,k) = quad_input_sequence(n,1);
        end
    end
end

quad_input_sequence = quad_input_sequence(n_output_units+1:end,:);
quad_output_sequence = quad_output_sequence(n_output_units+1:end,:);

% rescale for output
% config.preprocess = 'rescale';
% [quad_output_sequence] = featureNormailse(quad_output_sequence,config);
%    
% split data
quad_train_input_sequence = repmat(quad_input_sequence(1:sequence_length,:),1,n_input_units);
quad_test_input_sequence = repmat(quad_input_sequence(1+sequence_length:end,:),1,n_input_units);

quad_train_output_sequence = quad_output_sequence(1:sequence_length,:);
quad_test_output_sequence = quad_output_sequence(1+sequence_length:end,:);

% train
quad_states_train = config.assessFcn(individual,quad_train_input_sequence,config);
quad_output_weights = quad_train_output_sequence(config.wash_out+1:end,:)'*quad_states_train*inv(quad_states_train'*quad_states_train + config.reg_param*eye(size(quad_states_train'*quad_states_train)));
quad_Y_train = quad_states_train * quad_output_weights';

% test
quad_test_states =  config.assessFcn(individual,quad_test_input_sequence,config);
quad_Y_test = quad_test_states * quad_output_weights';

% calculate output
for i = 1:size(quad_Y_test,2)
     
    %nmse = (mean((quad_test_output_sequence(config.wash_out+1:end,i)-quad_Y_test(:,i)).^2)/var(quad_test_output_sequence(config.wash_out+1:end,i)));
     
    
    nmse = (mean((quad_test_output_sequence(config.wash_out+1:end,i)-quad_Y_test(:,i)).^2)/mean(quad_test_output_sequence(config.wash_out+1:end,i).^2));
     
    
    %nmse = (mean((quad_train_output_sequence(config.wash_out+1:end,i)-quad_Y_train(:,i)).^2)/var(quad_train_output_sequence(config.wash_out+1:end,i)));

    
     %quad_MC(i) = 1 - norm((quad_test_output_sequence(config.wash_out+1:end,i)-quad_Y_test(:,i)).^2)/ norm((quad_test_output_sequence(config.wash_out+1:end,i)-norm(quad_test_output_sequence(config.wash_out+1:end,i))).^2);

%      if sum(quad_Y_test(:,i)) == 0 || isnan(sum(quad_Y_test(:,i))) || isinf(sum(quad_Y_test(:,i)))
%          nmse = 1;
%      else
%          mse=norm(quad_test_output_sequence(config.wash_out+1:end,i)-quad_Y_test(:,i),2)^2/length(quad_test_output_sequence(config.wash_out+1:end,i));
%          sigEner=norm(quad_test_output_sequence(config.wash_out+1:end,i))^2;
%          nmse=(mse/sigEner);
%          
%          if nmse > 1
%              nmse = 1;
%          end
%      end
if nmse > 1
    nmse = 1;
end

quad_MC(i) = 1 - nmse;

%remove low values from measure
if quad_MC(i) <= 0.1
    quad_MC(i) = 0;
end
    
end
quad_MC = sum(quad_MC);

% quad_MC_k= 0; 
% test_in_var = quad_test_input_sequence(config.wash_out+1:end,1);
% targVar = 1/(length(test_in_var)-1) * sum((test_in_var-mean(test_in_var)).*(test_in_var-mean(test_in_var)));
% 
% for i = 1:n_output_units
%        
%     coVar = 1/(length(quad_Y(:,i))-1) * sum((quad_test_output_sequence(config.wash_out+1:end,i)-mean(quad_test_output_sequence(config.wash_out+1:end,i)))...
%        .*(quad_Y(:,i)-mean(quad_Y(:,i))));    
%     outVar = 1/(length(quad_Y(:,i))-1) * sum((quad_Y(:,i)-mean(quad_Y(:,i))).*(quad_Y(:,i)-mean(quad_Y(:,i))));    
%     totVar = (outVar*targVar(1));    
%     quad_MC_k(i) = (coVar*coVar)/totVar;
%     
% end
% 
% quad_MC_k(isnan(quad_MC_k)) = 0;
% quad_MC = sum(quad_MC_k);

end