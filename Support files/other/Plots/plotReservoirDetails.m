...
switch(config.dataset)
    ...
    case {'spiral','spiral_multi_class'}
        ...
        if best_individual.n_output_units == 2
            ...
            outData = interp2(reshape(x,size(x,1),size(x,2)), reshape(y,size(y,1),size(y,2)), reshape([zeros(config.wash_out,1); boundary_class],size(x,1),size(x,2)), X2, Y2, 'linear')-1;
            imagesc(outData*0.5);
            set(gca,'YDir','normal')
        else
    ...
    case {'boston_housing'} % for regression problems
        ...
        p = polyfit(config.test_output_sequence,test_sequence,1);
        x1 = linspace(0,1);
        y1 = polyval(p,x1);
        plot(x1,y1,'--','color','k')

        hold off
        err = calculateError(test_sequence,config.test_output_sequence,config);
        title(strcat('R^2 error:',num2str(1-err)));
    ...
    switch(config.res_type)
        ...
        case {'RoRmin','RoRminMTS','RoRminMTSplus','RoRGrid'}
            ...
            if contains(config.res_type,'RoRminMTS')%strcmp(config.res_type,'RoRminMTS')
                ...
                for f = 1:length(flag)
                    switch(flag(f))
                        ...
                        case 3 % plot state relaxation
                            ...
                            input = zeros(config.wash_out+150,config.task_num_inputs);
                ...
                states = evalData(best_individual,config,'train');
