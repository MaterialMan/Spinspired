...
function population = createIsing(config)
...
for pop_indx = 1:config.pop_size
    ...
    for i = 1:config.num_reservoirs
        ...
        population(pop_indx).time_period(i) = randi([1 1]); %not in use
       
        population(pop_indx).J(i) = 4*rand-2;
        population(pop_indx).kT(i) = randi([0 5]);
        population(pop_indx).probSpinUp(i) =  rand;
        population(pop_indx).init_spin{i}  = initSpins(config.num_nodes(i), population(pop_indx).probSpinUp);
