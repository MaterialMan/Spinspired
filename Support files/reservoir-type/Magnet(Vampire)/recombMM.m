...
if config.evolve_geometry 
    ...
    W= winner.time_steps_increment(:);
    L = loser.time_steps_increment(:);
    pos = randperm(length(L),sum(rand(length(L),1) < config.rec_rate));
    L(pos) = W(pos);
    ...
    for i = 1:config.num_reservoirs
        ...
        % input weights
        if size(winner.input_weights{i}) == size(loser.input_weights{i})
            ...
            loser.input_widths{i} = winner.input_widths{i};
        end
