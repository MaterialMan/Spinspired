...
for pop_indx = 1:config.pop_size
    ...
    %inputweights
    input_weights = getWeights(config.input_weight_initialisation...
        ,population(pop_indx).n_input_units+1, population(pop_indx).total_units,config.sparsity);
    
    % restrict number of inputs
    input_positions = randperm(population(pop_indx).total_units*(population(pop_indx).n_input_units+1),config.restricted_num_inputs);
    population(pop_indx).input_weights = zeros(population(pop_indx).n_input_units+1, population(pop_indx).total_units);
    population(pop_indx).input_weights(input_positions) = input_weights(input_positions);
