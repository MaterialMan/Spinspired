...
% mutate restricted number of input weights
input_weights = offspring.input_weights(:);
f = find(input_weights); % find outputs in use
pos = randperm(length(f),sum(rand(length(f),1) < config.mut_rate)); %get weights to delete
if rand < 0.5
    input_weights(f(pos)) = 0; % delete weight
end
...
if nnz(input_weights) > config.restricted_num_inputs
    inputs_pos = find(input_weights);
    while(nnz(input_weights) > config.restricted_num_inputs)
        input_weights(inputs_pos(randi([1 length(inputs_pos)]))) = 0;
    end
end
input_weights(input_weights<minimum_weight & input_weights~=0 & input_weights>-minimum_weight) = 0;
offspring.input_weights = reshape(input_weights,size(offspring.input_weights));
...
if config.restricted_num_outputs <= offspring.total_units
    restricted_output_mask = offspring.restricted_output_mask(:);
    f = find(restricted_output_mask); % find outputs in use
    pos = randperm(length(f),sum(rand(length(f),1) < config.mut_rate));
    if rand < 0.5
        restricted_output_mask(f(pos)) = 0;
    end
    %restricted_output_mask(randperm(length(restricted_output_mask),length(pos))) = 1;
    new_pos = randperm(length(restricted_output_mask),length(pos));
    restricted_output_mask(new_pos) = round(mutateWeight(restricted_output_mask(new_pos),[0 1],config));
    if nnz(restricted_output_mask) > config.restricted_num_outputs
        outputs_pos = find(restricted_output_mask);
        while(nnz(restricted_output_mask) > config.restricted_num_outputs)
            restricted_output_mask(outputs_pos(randi([1 length(outputs_pos)]))) = 0;
        end
    end
...
new_pos = randperm(length(indices),length(pos)); % find new positions to change
W(indices(new_pos)) = mutateWeight(W(indices(new_pos)),[-1 1],config); % mutate any random weights, chance to mutate existing and non-existing weight
