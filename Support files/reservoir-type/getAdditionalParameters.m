...
switch(res_type)
    ...
    case {'RoR','RoRmin','RoRminMTS','RoRminMTSplus','RoRGrid'}
        ...
        % interpolation
        config.max_interpolation_length = 10;
    ...
    case {'MM','multiMM'}
        ...
        config.poly_num = 4;
        config.geometry_file =  '';                    %add specific geometry file, e.g. custom.geo
        ...
        % material properties
        config.material_to_test = 'Ni';			%get set parameters
     ...
     case 'STO'
        ...
        config.damping_parameter = [0.01, 0.01];                 %Jed: [0.01, 0.01]
        ...
        config.applied_field_strength = [0.05 0.25];               %Jed: [0.03 0.25]
        config.initial_spin_direction = {'1,0,0'};

        config.spin_transfer_unit_vector = {'1,0,0'};
        config.spin_transfer_relaxation_torque = [-0.05, -0.05];    %Jed: [-.5e-4, -.5] factor 100 or 10. 0 to 1
        config.spin_transfer_precession_torque = [0.001, 0.001];    %Jed: [0.001, 0.001] . 0 to 1
        config.spin_transfer_torque_asymmetry  = [0.6, 0.6];        %Jed: [0.6, 0.6]. 0 to 1
