...
function population = MicrobialGA(ID,MM_size,task)
...
%if isempty(gcp) && config.parallel
 %   parpool('local',1,'IdleTimeout', Inf); % create parallel pool
%end

% type of network to evolve
config.res_type = 'multiMM';             % state type of reservoir(s) to use. E.g. 'RoR' (Reservoir-of-reservoirs/ESNs), 'ELM' (Extreme learning machine), 'Graph' (graph network with multiple functions), 'DL' (delay line reservoir) etc. Check 'selectReservoirType.m' for more.
config.num_nodes = {[MM_size]};                   % num of nodes in each sub-reservoir, e.g. if config.num_nodes = [10,5,15], there would be 3 sub-reservoirs with 10, 5 and 15 nodes each.
...
config.total_gens = 230;                    % number of generations to evolve
...
config.num_sync_offspring = 12;%config.deme;    % length of cycle/synchronisation step
...
for test = 1:config.num_tests
    ...
    clearvars -except config test best best_indv store_error population ID MM_size task
    ...
    for gen = 2:config.total_gens
        ...
        if config.multi_offspring
            ...
            % print info
            if (mod(gen,config.gen_print) == 0) 
                fprintf('Gen %d, time taken: %.4f sec(s)\n  Winner: %.4f, Loser: %.4f, Best Error: %.4f \n',gen,toc/config.gen_print,getError('test',population(winner)),getError('test',population(loser)),getError('test',population(best_indv(test,gen))));
                tic;
        ...
        if mod(gen,config.save_gen) == 0
            saveData(population,store_error,config,ID)
        end
...
function saveData(population,store_error,config,test)
config.figure_array =[];
...
save(strcat('Task_',config.dataset, res_type,'_run',num2str(test),'_gens',num2str(config.total_gens),'_',num2str(sum(config.num_reservoirs)),'Nres.mat'),...
