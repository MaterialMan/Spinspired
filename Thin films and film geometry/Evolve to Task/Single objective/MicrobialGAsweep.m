...
function avg_best = MicrobialGAsweep(ID,mut_rate,rec_rate,deme_percent)
...
config.mut_rate = mut_rate;                       % mutation rate
config.deme_percent = deme_percent;                   % speciation percentage; determines interbreeding distance on a ring.
config.deme = round(config.pop_size*config.deme_percent);
config.rec_rate = rec_rate;                       % recombination rate
...
for test = 1:config.num_tests
    ...
    clearvars -except config test best best_indv store_error population ID MM_size
