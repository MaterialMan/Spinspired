...
%         drawnow
        ...
        size_in_use = size_list(size_indx);
        size_div_in_use =size_div(dim_indx);
        ...
        parfor pop_indx = 1:config.pop_size
            ...
            fprintf('Size: %d, width: %.2f, height: %.2f, pos: %d, error: %.4f, num cells: %d\n', size_in_use,size_in_use/size_div_in_use,size_div_in_use,pop_indx,error(pop_indx),num_states(pop_indx))
        ...
        writetable(T,strcat('rectData2_',task,'_',num2str(pop_size),'_',num2str(ID),'.csv'),'Delimiter',',','QuoteStrings',true);
